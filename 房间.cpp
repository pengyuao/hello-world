...
static float lightPosition[] = { 0.0f, 0.0f, 5.0f, 0.0f };
GLuint textures[2];
...
void init(void) 
{
	...
	pBytes = gltReadBMPBits("door.bmp", &nWidth, &nHeight);
    glBindTexture(GL_TEXTURE_2D, textures[0]);	
	...
	pBytes = gltReadBMPBits("wall1.bmp", &nWidth, &nHeight);
	...
	free(pBytes);
		...
		pBytes = gltReadBMPBits("desk.bmp", &nWidth, &nHeight);
    glBindTexture(GL_TEXTURE_2D, textures[3]);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	...
	free(pBytes);
		...
		pBytes = gltReadBMPBits("bed.bmp", &nWidth, &nHeight);
    glBindTexture(GL_TEXTURE_2D, textures[4]);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
